...
export async function addSupabase() {
	...
	let layout: boolean | symbol = false;
	...
	const client = fs.readFileSync(
		new URL(
			`../assets/supabase/client${auth ? "Auth" : ""}.${language}`,
			import.meta.url,
		),
		"utf-8",
	);
	...
	if (auth) {
		...
		if (layout) {
			// +layout.svelte
			const layoutSvelte = fs.readFileSync(
				new URL(`../assets/supabase/layout${language}.svelte`, import.meta.url),
				"utf-8",
			);
			...
			const layoutClient = fs.readFileSync(
				new URL(`../assets/supabase/layout.${language}`, import.meta.url),
				"utf-8",
			);
			...
			// +layout.server
			const layoutServer = fs.readFileSync(
				new URL(
					`../assets/supabase/layout.server.${language}`,
					import.meta.url,
				),
				"utf-8",
			);
			...
			fs.appendFile(
				`src/routes/+layout.server.${language}`,
				layoutServer,
				(error) => {
					if (error) console.error(error);
				},
	...
	const environment = fs.readFileSync(
		new URL("../assets/supabase/environment.txt", import.meta.url),
		"utf-8",
	);
	...
	s.stop(
		"Installed " +
			kleur
				.bold()
				.blue(
					auth ? "@supabase/auth-helpers-sveltekit" : "@supabase/supabase-js",
				) +
			", created " +
			kleur
				.bold()
	...
	if (language === "ts") {
		...
		packJson.scripts[
			"update-types"
		] = `npx supabase gen types typescript --project-id "PROJECT_ID" --schema public > ${dbFolder}/types.ts`;
